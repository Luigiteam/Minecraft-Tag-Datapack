...
## Item 2 (Clamp Trap)
execute if score pow Rand matches 8..13 at @e[tag=positioned] run summon minecraft:item ~ ~280 ~ {Item:{id:"minecraft:polar_bear_spawn_egg",Count:1b,tag:{Floating:1b,EntityTag:{id:"minecraft:area_effect_cloud",Tags:[ClampTrap],Particle:"ambient_entity_effect 1 0 0 1",Age:0,WaitTime:0,Duration:6000,Radius:4.5f},display:{Name:'[{"text":"Clamp Trap","italic":false}]',Lore:['[{"text":"Right click on any block to set this trap up about 5","italic":false}]','[{"text":"block radius... But be careful, you may walk into your","italic":false}]','[{"text":"own trap...","italic":false}]']}}}}
execute if score pow Rand matches 8..13 at @e[tag=positioned] run summon minecraft:firework_rocket ~ ~10 ~ {LifeTime:20,FireworksItem:{id:"firework_rocket",Count:1,tag:{Fireworks:{Explosions:[{Type:1,Trail:1,Colors:[I;15435844],FadeColors:[I;11250603]}],Flight:1}}}}

## Item 3 (Potion of Invisibility)
execute if score pow Rand matches 0..7 at @e[tag=positioned] run summon minecraft:item ~ ~280 ~ {Item:{id:"minecraft:splash_potion",Count:1b,tag:{Floating:1b,Potion:"minecraft:water",CustomPotionEffects:[{Id:14,Duration:600,Ambient:1b}],display:{Name:'"Potion of Invisibility"',Lore:['[{"text":"This gives the user invisibility when applied, removing the glowing effect as well for a short time"}]']}}}}
...
execute if score pow Rand matches 14..15 run tellraw @a {"text":"A Powerup has spawned somewhere","color":"red"}

execute if score pow Rand matches 8..13 run tellraw @a {"text":"A Powerup has spawned somewhere","color":"gold"}

execute if score pow Rand matches 0..7 run tellraw @a "A Powerup has spawned somewhere"

execute if score pow Rand matches 16..20 run tellraw @a {"text":"A Powerup has spawned somewhere","color":"light_purple"}

execute if score pow Rand matches 21..27 run tellraw @a {"text":"A Powerup has spawned somewhere","color":"yellow"}
