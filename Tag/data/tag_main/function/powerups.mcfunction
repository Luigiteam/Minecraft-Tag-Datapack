...
## Item 1 (Wooden Sword)
$execute if score pow Rand matches $(kbStickMin)..$(kbStickMax) if score kbStick Toggle matches 1 run summon minecraft:item ^ ^280 ^$(distance) {Item:{id:"minecraft:wooden_sword",count:1b,components:{"minecraft:custom_data":{Floating:1b,Upgrade:0b},"minecraft:enchantments":{levels:{"minecraft:knockback":5}},"minecraft:damage":58,"minecraft:custom_name":'{"text":"Knockback Stick","color":"red","italic":false}'}}}
