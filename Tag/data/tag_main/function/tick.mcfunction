...
execute if score PowerupTimer gameTimer >= PowerupCooldown Numbers store result storage main random.rotation int 1 run random value -45..45

function tag_main:distance_random with storage main random

execute if score PowerupTimer gameTimer >= PowerupCooldown Numbers as @e[type=marker,tag=tpSpawn] at @s run function tag_main:powerups with storage minecraft:main random
...
execute if score effect Toggle matches 1 as @a[nbt={Inventory:[{id:"minecraft:potion",components:{"minecraft:custom_data":{Floating:1b,Effect:1b}}}]},scores={effectType=1..}] run scoreboard players set @s effectLost 0
...
execute if score State gameStart matches 1.. as @a[tag=tagger,nbt=!{Inventory:[{id:"minecraft:glow_berries",components:{"minecraft:custom_data":{Effect:1b}}}]}] run scoreboard players set @s effectLostWarped 1
